...
function guess() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (guess.length != 5)
            throw new Error("oi dont do that"); // catch error if player has edited html
        let data = yield fetch("api/guess?" + new URLSearchParams({ token: token, guess: guess }))
            .then(response => {
            if (response.status == 200)
                return response.json(); // if all is good decode
            else
                return response.text(); // else get error
        }).then(thong => {
            if (typeof (thong) == "string") {
                toast(thong, 1000);
                return false;
            } // toast the error
            else
                return thong; // or just return the decoded data
        });
        if (!data)
...
function newGame() {
    return __awaiter(this, void 0, void 0, function* () {
        token = yield fetch("/api/newgame").then(response => response.text());
        setCookie("token", token, 10);
    });
}
function getGame() {
    return __awaiter(this, void 0, void 0, function* () {
        let data = yield fetch("/api/getgame?" + new URLSearchParams({ token: token }))
